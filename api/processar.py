...
Lê o modelo e a fonte como ARQUIVOS (api/modelo_base.png, api/EBGaramond.ttf).
Rotas: POST /api/processar (webhook Supabase) | GET (saúde)
...
def sb_lock(rid, tentativas):
    url = SB_URL + "/rest/v1/" + TABELA + "?id=eq." + str(rid) + "&status=in.(novo,erro)"
...
def sb_patch(rid, campos):
    url = SB_URL + "/rest/v1/" + TABELA + "?id=eq." + str(rid)
...
def sb_ensure_bucket():
    try:
        requests.post(SB_URL + "/storage/v1/bucket", headers=_h({"Content-Type": "application/json"}),
...
def enviar_email(destino, nome, pdf_bytes, nome_arq):
    ...
    if r.status_code >= 300:
        raise RuntimeError("Brevo " + str(r.status_code) + ": " + r.text[:300])
...
def processar(body):
    ...
    try:
        if not sb_lock(rid, tentativas):
            return {"ok": True, "status": "ignorado"}, 200
